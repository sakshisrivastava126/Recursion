...
private:
    bool isValid(vector<vector<int>>& grid, int r, int c, int n, int expVal){
        ...
        if(r<0 || c<0 || r>=n || c>=n || grid[r][c] != expVal){
            return false;     //checking row & col grid se bahar na jaye and 
        }                     //grid we are moving on uski value expecteced value hi ho  

        if(expVal == n*n -1) return true;  //if we have reached the last expectedValue we done the work return true
